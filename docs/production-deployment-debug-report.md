...
1. **Next.js 16 Edge Proxy / Middleware (`proxy.ts`):** Next.js 16 App Router uses `proxy.ts` at the project root as the official proxy/middleware convention. Next.js 16 compiles `proxy.ts` directly as `ƒ Proxy (Middleware)`. Having both `middleware.ts` and `proxy.ts` creates a conflict in Next.js 16, so `proxy.ts` is the single authoritative proxy entrypoint.
